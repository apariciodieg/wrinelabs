          ...
          <dl className="flex flex-col items-center space-y-10 md:grid md:grid-cols-2 md:gap-x-20 md:gap-y-10 md:space-y-0">
            {features.map((feature) => (
              <div key={feature.name} className="relative">
                <dt>
                  <div className="absolute flex items-center justify-center w-12 h-12 text-slate-800 border-[1px] rounded-lg">
                    <feature.icon className="w-6 h-6" aria-hidden="true" />
                  </div>
                  <p className="ml-16 text-lg font-semibold leading-6 text-slate-800">{feature.name}</p>
                </dt>
